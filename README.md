...
公開される SMB share は **`yozist` 1 つだけ**（全仮想ビューへの単一エントリ）。`smb://host/yozist/` に接続すると、組込みビューと各「条件付きパス」（任意名）が仮想フォルダとして並び、配下を辿って各ビューへアクセスする。

| パス | 内容 |
|------|------|
| `yozist\` | ルート。組込みビューと各条件付きパス（任意名）が仮想フォルダとして並ぶ |
| `yozist\all\` | 全ファイルをフラット |
| `yozist\tags\仕事\2026\` | 階層パス = タグの **AND 条件**（「仕事 AND 2026」のファイル群） |
| `yozist\series\` | 配下に `NNNN__name` 形式で順序付きメンバー |
| `yozist\queries\<名前>\` | 全条件付きパスをまとめて公開（読取専用） |
| `yozist\<任意の名前>\` | **条件付きパス**。タグの AND / NOT 条件に任意の名前を付けたファイル群（読取専用） |

**条件付きパス**の名前・条件は作成したユーザーが WebUI からいつでも変更できる。作成・編集・削除は専用の **条件付きパスページ (`/ui/queries`)** で行い、DB を都度参照するハブ share へ即時反映される。Explorer のドラッグ＆ドロップでタグ付けが完結する。
