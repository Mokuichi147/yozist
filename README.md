...
| Share | 内容 |
|-------|------|
| `yozist` | **全仮想ビューへの単一エントリ（ハブ）**。`smb://host/yozist/` に接続すると、`all` / `tags` / `series` / `queries` と各「条件付きパス」（任意名）が仮想フォルダとして並ぶ。share 名を都度指定せずに 1 接続で全ビューを辿れる |
| `<任意の名前>` | **条件付きパス**。タグの AND / NOT 条件にマッチするファイル群へ任意の名前を付けて公開。`smb://host/<名前>/` で直接アクセスでき、名前・条件は作成したユーザーが WebUI からいつでも変更できる（読取専用） |
| `tags` | 階層パス = タグの **AND 条件**。`tags\仕事\2026\` で「仕事 AND 2026」のファイル群 |
| `series` | 配下に `NNNN__name` 形式で順序付きメンバー |
| `recent` | 直近 N 件（読取専用） |
| `all` | 全ファイルをフラット |
| `queries` | 全条件付きパスを `queries\<名前>\` の配下にまとめて公開（読取専用） |

Explorer のドラッグ＆ドロップでタグ付けが完結する。条件付きパスの作成・編集・削除は WebUI の **管理ページ (`/ui/manage`)** で行い、SMB の任意名 share へ即時に反映される。
