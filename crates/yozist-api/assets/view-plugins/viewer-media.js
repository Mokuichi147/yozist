...
(() => {
  ...
  ViewRuntime.registerView({ kind: 'core/image', async mount(cont, ctx) {
  ...
  ViewRuntime.registerView({ kind: 'media/video', async mount(cont, ctx) {
    const url = await ctx.objectUrlFor(ctx.mime);
    cont.innerHTML = `<video src="${url}" controls
      class="max-w-full max-h-[70vh] mx-auto block rounded bg-black"></video>`;
  } });
  ViewRuntime.registerView({ kind: 'media/audio', async mount(cont, ctx) {
    const url = await ctx.objectUrlFor(ctx.mime);
    cont.innerHTML = `<audio src="${url}" controls class="w-full mt-2"></audio>`;
  } });
  ViewRuntime.registerView({ kind: 'doc/pdf', async mount(cont, ctx) {
    const url = await ctx.objectUrlFor(ctx.mime);
